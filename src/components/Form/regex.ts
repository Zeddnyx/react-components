export const USERVALIDATION = /^(?=^[ -~]{6,10}$)[a-zA-Z\-\-0-9]+$/; // only letters and numbers
export const EMAILVALIDATION = /^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$/;
export const PASSVALIDATION = /^(?=^[ -~]{6,16}$)(?=.*([a-z][A-Z])).+$/; // min 6 max 16, min 1 lowwercase min 1 uppercase
...
export const userValidation = (username: string, error: boolean) => {
  return error && username.length <= 0
    ...
    : error && !username.match(USERVALIDATION)
...
export const emailValidation = (email: string, error: boolean) => {
  return error && email.length <= 0
    ...
    : error && !email.match(EMAILVALIDATION)
    ? "email must be valid"
    : "";
};
...
export const passwordValidation = (password: string, error: boolean) => {
  return error && password.length <= 0
    ...
    : error && !password.match(PASSVALIDATION)
